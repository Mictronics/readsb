...
namespace READSB {
    ...
    function SvgPathToSvg(path: string, stroke: string, fill: string, selectedStroke: string): string {
        path = path.replace("aircraft_color_fill", fill).replace("aircraft_color_stroke", stroke).replace("add_stroke_selected", selectedStroke);
        return path;
    }

    export function SvgPathToUri(path: string, stroke: string, fill: string, selectedStroke: string): string {
        return "data:image/svg+xml;base64," + btoa(SvgPathToSvg(path, stroke, fill, selectedStroke));
    }
}
